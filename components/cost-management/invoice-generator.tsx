...
export function InvoiceGenerator({
	...
	function handleSubmit(values: z.infer<typeof formSchema>) {
		const selectedCars = cars.filter((car) =>
			values.carIds?.includes(car.ID || "")
		);
		const selectedCosts = costs.filter(
			(cost) =>
				values.costIds &&
				values.costIds.includes(cost.car_id.toString())
		);
		const totalAmount =
			selectedCars.reduce((sum, car) => sum + 10, 0) +
		...
		const invoice: Invoice = {
			...
			buyerName:
				buyers.find((buyer) => buyer.id === values.buyerId)?.name ||
				"",
			cars: selectedCars,
			costs: selectedCosts,
			totalAmount,
					...
					render={() => (
						<FormItem>
							<div className="mb-4">
								...
								<FormDescription>
									Select the cars to include in the
									invoice.
								</FormDescription>
							</div>
							{cars.map((car) => (
									...
									render={({ field }) => {
										return (
											...
											>
												<FormControl>
													<Checkbox
														// checked={field.value && field.value.includes(car.ID) ? true : false}
														onCheckedChange={(
															checked
														) => {
															return checked
					...
					render={() => (
						<FormItem>
							<div className="mb-4">
								...
								<FormDescription>
									Select any additional costs to
									...
									render={({ field }) => {
										return (
											...
											>
												<FormControl>
													<Checkbox
														// checked={field.value?.includes(cost.car_id)}
														onCheckedChange={(
															checked
														) => {
															return checked
