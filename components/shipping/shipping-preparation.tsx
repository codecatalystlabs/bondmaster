...
const containers: ShippingContainer[] = [
	...
	},
	{
		...
		maxCapacity: 6,
		dimensions: { length: 12.2, width: 2.44, height: 2.59 },
		maxWeight: 30480,
	},
];
...
export function ShippingPreparation() {
	...
	function onSubmit(values: z.infer<typeof formSchema>) {
		const selectedItems: ShippingOrderItem[] = values.items.map(
			(item) => ({
		...
		const selectedContainer = containers.find(
			(container) => container.id === values.container
		);

		if (selectedContainer) {
			const newOrder: ShippingOrder = {
				id: Math.random().toString(36).substr(2, 9),
				orderNumber: `SO-${Math.random()
					.toString(36)
					.substr(2, 6)
					.toUpperCase()}`,
				items: selectedItems,
	...
	const calculateTotalVolume = (items: ShippingOrderItem[]) => {
		...
		return items.reduce(
			(total, item) => total + item.quantity * carVolume,
			0
		);
	};
